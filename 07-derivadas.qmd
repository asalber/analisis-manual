...
a. Derivada de la suma de funciones.
\begin{align*}
...
b. Derivada de la resta de funciones. Se prueba del mismo modo que la suma.

b. Derivada del producto de una función por un escalar.
\begin{align*}
...
c. Derivada del producto de funciones.
\begin{align*}
...
d. Derivada del cociente de funciones.
\begin{align*}
...
## Demostración
:::{.proof}
Como $f$  es continua en $I$, $J=f(I)$ es un intervalo. Como además $f$ es inyectiva, necesariamente $f$ es monótona. Sea la función $g(y)=\frac{y-f(a)}{f^{-1}(y)-a}$ $\forall y\in J\setminus \{f(a)\}$. $g$ está bien definida pues $f^{-1}$ es inyectiva, y además, si $y\neq f(a)$ entonces $f^{-1}(y) \neq f^{-1}(f(a))=a$, por lo que el denominador no se anula. Veamos que $\lim_{y\to f(a)}g(y)=f'(a)$.
...
## Demostración
:::{.proof}
Si $f$ es diferenciable en $I$ entonces es continua en $I$ y se puede aplicar el teorema anterior.
:::
:::
...
Hasta hora siempre hemos trabajado con funciones de la forma $y=f(x)$ donde la variable $y$ depende de la variable $x$ según la función $f(x)$. Esta representación se conoce como explícita, por que la variable dependiente $y$ aparece despejada en el lado izquierdo de la igualdad. Sin embargo, como ya se vió en la @def-funcion, una una función real de variable real es una relación formada por pares $(x,y)\in \mathbb{R}\times\mathbb{R}$, de modo que también se puede representar de manera intensiva mediante una ecuación $F(x,y)=0$, que cumplen los puntos de la función y solo ellos. 
...
## Demostración
:::{.proof}
Sea $g(x)$ la recta secante a $f$ en los puntos $(a,f(a))$ y $(b,f(b))$. Entonces $g(x)=f(a)+\operatorname{TVM}(f,[a,b])(x-a) = f(a)+\frac{f(b)-f(a)}{b-a}(x-a)$. Si tomamos la función que mide la distancia entre $f$ y $g$, es decir, $\forall x\in[a,b]$
...
Por tanto, aplicando el teorema de Rolle, existe $c\in(a,b)$ tal que $f'(c)=0$. Como 
...
## Demostración
:::{.proof}
Probaremos solo el primer apartado, ya que el segundo se prueba de forma análoga.
...
## Demostración
:::{.proof}
Demostraremos solo el caso de un máximo, ya que el otro caso es análogo. Para ver que $f$ tiene un máximo local en $c$ basta con probar que si $x\in(c-\delta,c+\delta)$ entonces $f(x)\leq f(c)$.
...
## Demostración
:::{.proof}
Definimos $g(x)=k(x-a)-f(x)$ $\forall x\in[a,b]$. $g$ es diferenciable en $[a,b]$ al serlo $f$ y $g'(x)=k-f'(x)$ $\forall x\in[a,b]$. Por tanto, $g$ es continua en $[a,b]$, y entonces tiene un máximo y un mínimo relativos en $[a,b]$.
...
Ya se vió, por el @thm-extremos-funcion-continua-intervalo-cerrado, que una función continua en un intervalo cerrado $[a,b]$ alcanza el máximo y el mínimo absolutos en ese intervalo. Así pues, para encontrar los extremos absolutos de una función $f$ derivable en $[a,b]$, basta con seguir el siguiente procedimiento:
...
## Demostración
:::{.proof}
Daremos un prueba informal del primer apartado, ya que el segundo se prueba de manera análoga por simetría, ya que si $f$ es cóncava hacia abajo, $-f$ es cóncava hacia arriba.
...
<!-- Probaremos solamente el punto primer apartado ya que el segundo se prueba de manera análoga por simetría, ya que si $f$ es cóncava hacia abajo, $-f$ es cóncava hacia arriba.

Supongamos que $f$ es cóncava hacia arriba. Para cualquier $x\in I$, como existe $f''(x)$, se tiene que 

$$
f''(x) = \lim_{h\to 0}\frac{f(x+h-2f(x)+f(x-h))}{h^2}.
$$

Tomando $h\in\mathbb{R}$ tal que $x+h, x-h \in I$, como $f$ es cóncava hacia arriba en $I$, se tiene que 

$$
\begin{gathered}
f(x) = f(\frac{1}{2}(x+h)+(x-h)) \leq \frac{1}{2}f(x+h)+\frac{1}{2}f(x-h) \\ 
\Rightarrow f(x+h)+f(x-h)-2f(x) \geq 0 \Rightarrow \frac{f(x+h)-f(x-h)-2f(x)}{h^2} \geq 0 \Rightarrow f''(x)\geq 0$
\end{gathered}
$$ -->
:::
:::
