...
En este capítulo se estudian las _series_ de números reales, un tipo especial de sucesiones que se construyen a partir de la suma de los términos de otras sucesiones. Este tipo de sucesiones son las más importantes del Análisis pues, como se verá más adelante, aparecen en multitud de contextos reales. 

Como ejemplo introductorio puede servir la [paradoja de la dicotomía de Zenon](https://es.wikipedia.org/wiki/Paradojas_de_Zen%C3%B3n#Paradoja_de_la_dicotom%C3%ADa), que establece que para que un corredor pueda recorrer una distancia hasta la meta, primero tiene que recorrer la mitad de la distancia, después la mitad de la distancia restante, después la mitad de la distancia restante, y así hasta el infinito, por lo que, aparentemente, nunca llegaría a la meta.

![Paradoja de la dicotomía de Zenon.(imagen tomada de Wikipedia)](img/series/paradoja-dicotomia-zenon.png)
...
Esta serie se conoce como _serie armónica_. En la siguiente gráfica se puede apreciar cómo evoluciona la sucesión de sus sumas parciales.

::: {.content-visible when-format="html"}
<div id="ggb-serie-armonica"></div>
...
var params = {
            ...
            "material_id":"efjzy9kw"
            };
    var ggbApplet1 = new GGBApplet(params, true);
    window.addEventListener("load", function() { 
        ggbApplet1.inject('ggb-serie-armonica');
    });
</script>
:::
:::

En ocasiones es posible expresar el valor de la suma parcial de orden $n$ mediante una fórmula explícita que depende de $n$ y que se conoce como _forma cerrada_ de la serie. Si una serie puede expresarse mediante una forma cerrada, resulta más rápido calcular el valor de la suma parcial de orden $n$ mediante la forma cerrada, que haciendo la propia suma de los $n$ primeros términos de la sucesión.
...
::: {.content-visible when-format="html"}
<div id="ggb-series-paradoja-zenon"></div>
...
var params = {
            ...
            //"showFullscreenButton": true,
            "transparentGraphics": true,
            "material_id":"nh59tcxp"
            };
    var ggbApplet2 = new GGBApplet(params, true);
    window.addEventListener("load", function() { 
        ggbApplet2.inject('ggb-series-paradoja-zenon');
    });
</script>
:::
:::
...
## Demostración
:::{.proof}
Pongamos $A_n=a_1+a_2+\cdots+a_n$, $B_n=b_1+b_2+\cdots+b_n$, $\alpha=\sum_{i=1}^{k}a_i$, $\beta=\sum_{i=1}^{k}b_i$. Las afirmaciones hechas se deducen todas de que para todo $n\ge q+1$ se verifica la igualdad:
...
## Demostración
:::{.proof}
Sean $(A_n)_{n=1}^\infty$ y $(B_n)_{n=1}^\infty$ las sucesiones de las sumas parciales de $\sum a_n$ y $\sum b_n$ respectivamente. Si $\sum a_n$ y $\sum b_n$ convergen, entonces $(A_n)_{n=1}^\infty$ y $(B_n)_{n=1}^\infty$ convergen y, por las propiedades de las sucesiones convergentes, $(A_n+B_n)_{n=1}^\infty$ converge, y además

$$
\lim_{n\to\infty} A_n+B_n = \lim_{n\to\infty}A_n+\lim_{n\to\infty}B_n.
$$

En consecuencia, $\sum (a_n+b_n)$ converge y $\sum_{n=1}^\infty (a_n+b_n) = \sum_{n=1}^\infty a_n+\sum_{n=1}^\infty b_n$.
:::
:::
...
## Demostración
:::{.proof}
Sea $(A_n)_{n=1}^\infty$ la sucesión de las sumas parciales de $\sum a_n$. Entonces, $\sum a_n$ converge si y solo si $(A_n)_{n=1}^\infty$ converge, y por el criterio de convergencia de Cauchy para sucesiones (@thm-criterio-convergencia-cauchy) esto es equivalente a que $(A_n)_{n=1}^\infty$ es una sucesión de Cauchy, de manera que se cumple que para cualquier $\varepsilon>0$ existe un $k\in\mathbb{N}$ tal que $|A_m-A_n|<\varepsilon$ $\forall m>n\geq k$, lo que es equivalente a que $|a_{n+1}+a_{n+2}+\cdots+a_m|<\varepsilon\ \forall m>n\geq k$.
:::
:::
...
Sea $(A_n)_{n=1}^\infty$ la sucesión de las sumas parciales de de $\sum a_n$. Si la serie $\sum a_n$ es convergente, entonces existe $\lim_{n\to\infty}A_n=l$, y por las propiedades de las colas de sucesiones, también se cumple que $\lim_{n\to\infty}A_{n-1}=l$. 

Por otro lado, como $a_n=A_n-A_{n-1}$ $\forall n\geq 2$, se deduce que 
...
## Demostración
:::{.proof}
La demostración es una consecuencia inmediata del criterio de divergencia, ya que si $\sum a_n$ converge, entonces $\lim_{n\to\infty} a_n = 0$ y, por tanto, $\lim_{n\to\infty} a_n^{-1} = \frac{1}{\lim_{n\to\infty} a_n} = \infty$, por lo que la serie $\sum a_n^{-1}$ diverge.
:::
:::
...
## Demostración
:::{.proof}
La demostración es fácil a partir de la proposición anterior y se deja como ejercicio.
:::
:::
...
## Demostración
:::{.proof}
Usando el criterio de divergencia, es fácil probar que la serie diverge para $p\leq 0$, ya que $\lim_{n\to\infty} \frac{1}{n^p}=\infty$ si $p<0$ y $\lim_{n\to\infty} \frac{1}{n^p}=1$ si $p=0$.
...
Ya se ha visto que la serie armónica $\sum \frac{1}{n}$ diverge, ya que $p=1$, mientras que la serie $\sum \frac{1}{n^2}$ converge al ser $p=2>1$. De hecho, la suma exacta de esta última serie es el famoso [problema de Basilea](https://es.wikipedia.org/wiki/Problema_de_Basilea) que consiguió resolver Euler, demostrando que que $\sum_{n=1}^\infty \frac{1}{n^2} = \frac{\pi}{6}$ (la demostración se escapa de los conocimientos de este curso y puede verse en el enlace anterior).
:::

[![Graficador de Series](img/logos/logo-geogebra.png)](https://www.geogebra.org/classic/e4ynyzhn)
...
Si la serie es de términos positivos, entonces $|a_n|=a_n$ $\forall n\in\mathbb{N}$ y, por tanto, la convergencia y la convergencia absoluta son equivalentes. Pero si la serie tiene términos negativos, puede ocurrir que la serie sea convergente pero no absolutamente convergente.
...
:::{.callout-warning}
$\lim_{n\to\infty}\left|\frac{a_{n+1}}{a_n}\right|=l>1$, el criterio de la razón no garantiza nada, es decir, la serie podría ser convergente o divergente.
:::
...
:::{#-exm-serie-funcional}
Las primeras sumas funcionales parciales de la serie funcional $\sum \frac{x}{n}$ son 
...
La sucesión $(c_n)_{n=0}^\infty$ se llama _sucesión de coeficientes de la serie_, y al término $c_0$ se le llama _termino independiente_.
:::
...
Para cualquier serie de potencias $\sum c_n(x-a)^n$, está claro que $a$ pertenece al dominio de convergencia puntual de la serie, pues para $x=a$ se tiene $\sum_{n=1}^\infty c_n(a-a)^n = c_0$. Veremos a continuación, para qué otros valores de $x$ la serie de potencias converge.
 
:::{#def-radio-convergencia}
## Radio de convergencia de una serie de potencias
Dada una serie de potencias $\sum c_n(x-a)^n$, se define el _radio de convergencia_ de la serie como 
...
El siguiente teorema nos permite definir el dominio de convergencia puntual de una serie de potencias.

:::{#thm-dominio-convergencia-serie-potencias}
Dada una serie de potencias $\sum c_n(x-a)^n$ con radio de convergencia $R$, se cumple

a. La serie converge para todo $|x-a|<R$.
b. La serie diverge para todo $|x-a|>R$.
:::

:::{.callout-note collapse="true"}
## Demostración
:::{.proof}
Aplicaremos el criterio de la raíz para estudiar la convergencia absoluta de la serie $\sum c_n(x-a)^n$.

$$
\lim_{n\to\infty} \sqrt[n]{|c_n(x-a)^n|} = \lim_{n\to\infty} \sqrt[n]{|c_n|}\sqrt[n]{|(x-a)|^n} = \frac{|x-a|}{R}.
$$

Por tanto, la serie converge si $\frac{|x-a|}{R}<1$, es decir, si $|x-a|<R$, y diverge si $|x-a|<R$.
:::
:::

:::{.callout-warning}
El teorema anterior no afirma nada sobre los puntos $|x-a|=R$. De hecho, en estos los puntos la serie puede converger o divergir. 
:::

:::{#exm-dominio-convergencia-serie-potencias}
Veamos cuál es el dominio de convergencia puntual de la serie $\sum \frac{(x-2)^n}{n}$. El radio de convergencia de la serie es 

$$
R= \frac{1}{\lim_{n\to\infty} \sqrt[n]{|\frac{1}{n}|}} = \frac{1}{1} = 1
$$

Por tanto, la serie converge para todo $|x-2|<1$, es decir, $1<x<3$. En $x=1$ y $x=3$ el teorema anterior no dice nada, pero podemos estudiar la convergencia de estos dos casos particulares. En $x=1$ tenemos la serie $\sum \frac{(-1)^n}{n}$ que es la serie armónica alternada, y por tanto, converge. Y en $x=3$ tenemos la serie $\sum \frac{1}{n}$ que es la serie armónica, y por tanto diverge. 

Así pues, el dominio de convergencia puntal de la serie es el intervalo $[1,3)$.
:::
...
Ya hemos dicho que el límite, $L$, de una serie convergente, $L=\lim\ser{a}$, no es, como a veces se dice, una ``suma de los infinitos términos'' de la sucesión $(a_n)_{n=1}^\infty$. ¿Qué sentido tiene eso de ``sumar infinitos términos''? Ninguno, desde luego. Lo que  dicho número verifica es que $\modulo{L-\sum_{j=1}^{n}a_j}$ se conserva menor que cualquier número \epos, a partir de un cierto   \nN\ en adelante. Si bien, puede ser sugerente la interpretación de $\,L\,$ como ``la suma de los términos de la sucesión $(a_n)_{n=1}^\infty$'', no hay que olvidar que esto  no es más que una forma de hablar, y  que el límite de una serie convergente es, justamente, el límite de una sucesión de sumas y no debe confundirse con una operación algebraica. Por ello cabe preguntarse si las propiedades asociativa y conmutativa de la adición se conservan para series convergentes. De hecho, ya hemos visto que la propiedad conmutativa no se verifica en general, pues reordenando los términos de una serie convergente podemos obtener otra serie con suma distinta. Las cosas van mejor en lo que se refiere a la asociatividad. Precisemos estas ideas.

Sea \ser{a}\ la serie definida por la sucesión $(a_n)_{n=1}^\infty$. Dada una aplicación estrictamente creciente $\sigma:\N\rightarrow\N$, definamos una sucesión $(b_n)_{n=1}^\infty$\ por:
...
\mbox{$\,A_n=a_1+a_2+\cdots+a_n$,} y $B_n=b_1+b_2+\cdots+b_n$, se tiene que $B_n=A_{\sigma(n)}$, es decir la sucesión $(B_n)_{n=1}^\infty$\ es una sucesión parcial de $(A_n)_{n=1}^\infty$. Deducimos el siguiente resultado.
...
Es importante advertir que asociando términos en una serie no convergente puede obtenerse una serie convergente. Por ejemplo, la serie definida por la sucesión $$(a_n)_{n=1}^\infty$=\suc{(-1)^{n+1}}$ no es convergente, y la serie que se obtiene de ella asociando términos dos a dos, es decir, la serie definida por la sucesión $b_n=a_{2n-1}+a_{2n}=0$, es evidentemente convergente. A este respecto tiene interés el siguiente resultado que establece una condición suficiente para que de la convergencia de una serie obtenida asociando términos en otra pueda deducirse la convergencia de esta última.

\begin{proposicion}\label{prop:asociabien} Sea $\sigma:\N\rightarrow\N$ una aplicación estrictamente creciente, $(a_n)_{n=1}^\infty$\ una sucesión y $(b_n)_{n=1}^\infty$\ la sucesión definida como en \eqref{eq:asociandoterminos}. Supongamos que la serie $\sum b_n$ es convergente y que la sucesión
$$
...
$$
De donde se sigue que $\lim$(A_n)_{n=1}^\infty$=\lim\big\{B_{\tau(n)}\big\}=\lim$(B_n)_{n=1}^\infty$$.\fin
...
Sea \ser{a}\ la serie definida por la sucesión $(a_n)_{n=1}^\infty$. Dada una biyección
$\pi :\N\rightarrow\N$, definamos una sucesión $(b_n)_{n=1}^\infty$\ por
...
Probaremos en primer lugar que la serie \ser{a}\ también es convergente. Dado \epos, la condición de Cauchy para $(B_n)_{n=1}^\infty$\ nos dice que existe $n_0\en\N$ tal que
...
\sum_{k=p+1}^{q}|a_k|<\frac{\eps}{2}<\eps.$$ Lo que prueba que la serie $(A_n)_{n=1}^\infty$\ cumple
la condición de Cauchy y, por tanto, es convergente.

Pongamos $\,A=\lim$(A_n)_{n=1}^\infty$$, y sea $\pi:\N\rightarrow\N$ una biyección. Dado \epos, sea $\,n_0\en\N$ tal que se
...
$a) \longrightarrow b)$\ Probaremos que si la serie $(B_n)_{n=1}^\infty$\ no es
convergente entonces la serie $(A_n)_{n=1}^\infty$\ no es conmutativamente convergente.
Supondremos, pues, en lo que sigue que $(B_n)_{n=1}^\infty$\ no es convergente. Tenemos
para la serie $(A_n)_{n=1}^\infty$\ dos posibilidades: o bien converge o bien no converge.
Evidentemente, si $(A_n)_{n=1}^\infty$\ no converge entonces, con mayor razón, no es
conmutativamente convergente. Consideraremos, por tanto, el caso en que
$(A_n)_{n=1}^\infty$\ es convergente. Para nuestro propósito es suficiente probar que, en
...
positivos consecutivos de la sucesión $(a_n)_{n=1}^\infty$\ y, entre cada dos de tales
...
\propuesto Sea $(a_n)_{n=1}^\infty$\ una sucesión decreciente de números positivos y supongamos que la serie $\sum a_n$ es convergente. Prueba que $\suc{na_n}$ converge a $0$.
...
\end{proposicion}
\dem Pongamos $\,A_n=a_1+a_2+\cdots+a_n$, $B_n=b_1+b_2+\cdots+b_n$. Las hipótesis hechas implican que para todo $n>k$ es $\,A_n\!\leqslant B_n+A_k$. Deducimos que si $(B_n)_{n=1}^\infty$\ está mayorada también lo está $(A_n)_{n=1}^\infty$. \fin
...
divergentes.
\end{enumerate}
En particular, si dos sucesiones de números positivos, $(a_n)_{n=1}^\infty$\ y $(b_n)_{n=1}^\infty$\ son asintóticamente equivalentes, las respectivas series, $\sum a_n$ y $\sum b_n$ ambas convergen o ambas divergen.
\end{proposicion}
...
\begin{enumerate}
\item[i)] Si $$(L_n)_{n=1}^\infty$\to L\,$, donde $\,L>1\,$ o  $\,L=+\infty$, la serie \serien{a}\ es convergente.

\item[ii)] Si $$(L_n)_{n=1}^\infty$\to L\,$, donde $\,L<1\,$ o  $\,L=-\infty$, o bien si existe algún $\,k\en\N$ tal que $\,L_n\leqslant 1$ para todo $n\geqslant k$, entonces la serie \serien{a}\ es divergente.
\end{enumerate}
\end{proposicion}
...
\item[b)] Si $\,L>1\,$ o si $L=+\infinity$ o si hay un número $k\en\N$ tal que para todo $n\!\ge\!k$ es $\frac{a_{n+1}}{a_n}\!\ge\!1$,  entonces \serien{a}\ es divergente y además $(a_n)_{n=1}^\infty$\ no converge a $0$.
\end{enumerate}
\end{proposicion}
...
$\,a_n\!\ge \dfrac{a_{n_0}}{\lambda^{n_0}}\lambda^{n}$. Como $\lambda>1$ se sigue que la sucesión $(a_n)_{n=1}^\infty$\ diverge positivamente y, con mayor razón, la serie \serien{a}\ diverge positivamente.\hfill$\Box$
...
$$
El criterio del cociente nos dice que si $\dfrac{x^2}{4}<1$, es decir, $\abs{x}<2$, la serie es convergente; si  $\dfrac{x^2}{4}>1$, es decir, $\abs{x}>2$, la serie no es convergente porque $(a_n)_{n=1}^\infty$\ no converge a $0$. El caso en que $x^2=4$, o sea $x=\pm 2$, se tiene que:
$$
...
\begin{enumerate}
\item[a)] Si $\,L<1\,$ la serie \serien{a}\ es convergente.
\item[b)] Si $\,L>1\,$ o si $L=+\infinity$ o o si hay un número $k\en\N$ tal que para todo $n\!\ge\!k$ es $\sqrt[n]{a_{n}}\!\ge\!1$ entonces \serien{a}\ es divergente y además $(a_n)_{n=1}^\infty$\ no converge a $0$.
\end{enumerate}
\end{proposicion}
...
b) Si $L>1$ entonces, tomando $\lambda$ tal que $1<\lambda<L$ y razonando como antes, obtenemos que para todo $n\ge n_0$ es $a_n\!\ge\lambda^{n}$ y, como $\lambda>1$, se sigue que la sucesión $(a_n)_{n=1}^\infty$\ diverge positivamente y, con mayor razón, la serie
...
\begin{enumerate}
\item[i)] Si $$(R_n)_{n=1}^\infty$\to L$, donde $L>1$ o  $L=+\infinity$, la serie \serien{a}\ es convergente.

\item[ii)] Si $$(R_n)_{n=1}^\infty$\to L$, donde $L<1$ o  $L=-\infinity$, o bien si existe algún $k\en\N$ tal que $R_n\!\leqslant 1$ para todo $n\geqslant k$, entonces la serie \serien{a}\ es divergente.
\end{enumerate}
\end{proposicion}
...
\end{enumerate}
\end{proposicion}
Los criterios de convergencia que acabamos de estudiar hacen siempre hipótesis sobre la sucesión $(a_n)_{n=1}^\infty$\ para obtener información sobre el comportamiento de la serie \serien{a}. Ya dijimos antes que esto es típico del estudio de las series. Pero no lo olvides: no estamos estudiando la sucesión $(a_n)_{n=1}^\infty$\ sino la sucesión $\serien{a}=\suc{a_1+a_2+\dots+a_n}$.
...
\propuesto Sea $(a_n)_{n=1}^\infty$\ una sucesión creciente de números positivos. Dar condiciones que garanticen que la serie $\dis\sum_{n\ge 1}\dfrac{1}{a_1a_2a_3\cdots a_n}$ es convergente.

\propuesto Dar ejemplos de sucesiones $$(a_n)_{n=1}^\infty$\to 1$ y decrecientes tales que la serie  $\dis\sum_{n\ge 1}\dfrac{1}{a_1a_2a_3\cdots a_n}$ sea en un caso convergente y en otro caso divergente.
...
\lim_{n\to\infty}\dfrac{-\log(a_n)}{\log n}=\left\{
                          ...
                          \right.
$$
La serie converge si $\alpha>1$ y no converge si $\alpha<1$. Para $\alpha=1$ se tiene que $$(a_n)_{n=1}^\infty$\to\dfrac{1}{\e}$ y por tanto la serie no converge porque su término general no converge a $0$.\hecho

$m)$\ Pongamos $a_n=a^{\sum_{j=1}^{n}1/j}$. Es evidente que si $a\ge 1$ se tiene que $a_n\ge 1$ y, por tanto, la serie no es convergente porque $(a_n)_{n=1}^\infty$\ no converge a $0$. Podemos aplicar el criterio del cociente.
$$
...
Consideremos una serie del tipo \serie{(c_n)^\alpha}\ donde $c_n=\dfrac{p_1p_2\cdots p_n}{q_1q_2\cdots q_n}$ y $p_j, q_j$ son números enteros positivos. Además $q_n$ es de la forma $q_n=p_n+k$  donde $k$ es un entero positivo fijo. En el ejemplo que nos ocupa es $p_n=2n$ y $q_n=2n+3=p_n+3$. Observa que para que $$(c_n)_{n=1}^\infty$\to 0$ es necesario que $\alpha>0$. Una estrategia bastante buena para estudiar estas series consiste en acotar directamente $c_n$ usando la desigualdad (válida por ser $p_n<q_n$):
$$
...
\resuelto Sea $(a_n)_{n=1}^\infty$\ una sucesión creciente de números positivos. Dar condiciones que garanticen que la serie $\dis\sum_{n\ge 1}\dfrac{1}{a_1a_2a_3\cdots a_n}$ es convergente.

\sol Pongamos $x_n=\dfrac{1}{a_1a_2a_3\cdots a_n}$. Si $(a_n)_{n=1}^\infty$\ no está mayorada, como es creciente se tiene que $$(a_n)_{n=1}^\infty$\to +\infty$. Por tanto, hay un número $k\en\N$ tal que para todo $n\ge k$ se verifica que $a_n\ge 2$. Deducimos que para $n\ge k$ se verifica que:
$$
...
\sol Pongamos $b_n=\dfrac{a_n}{1+a_n}$. Como $1+a_n\ge 1$, la desigualdad $b_n\le a_n$ prueba que si la serie $\sum a_n$ es convergente también es convergente la serie $\sum b_n$. Recíprocamente, si la serie $\sum b_n$ es convergente entonces debe ser $$(b_n)_{n=1}^\infty$\to 0$, por lo que hay algún $k\en\N$ tal que para todo $n\ge k$ es $b_n<\frac{1}{2}$, esto es, $2a_n<1+a_n$ por lo que $a_n<1$. Lo que implica que $a_n^2<a_n$, y obtenemos que $a_n^2+a_n<2a_n$ de donde $a_n<\dfrac{2a_n}{1+a_n}=2b_n$. De esta desigualdad se sigue, por el criterio de comparación, que la serie $\sum a_n$ también es convergente.\hecho
...
\sol Como $$(a_n)_{n=1}^\infty$\to  0$, hay un número $k\en\N$ tal que para todo $n\ge k$ es $0<a_n<1$, lo que implica que $0<a_n^2<a_n$ y deducimos, por el criterio de comparación, que la serie $\sum a_n^2$ es convergente. Como
$$
...
Dadas dos sucesiones $(a_n)_{n=1}^\infty$\ y $(b_n)_{n=1}^\infty$, pongamos $\,A_p=\sumaf{j=1}{p}a_j$. Se
...
\begin{teorema}[\textbf{Criterio general de Dirichlet}]\label{th:cgd} Sean $(a_n)_{n=1}^\infty$\,, $(b_n)_{n=1}^\infty$\ dos sucesiones, y
...
\begin{teorema}[\textbf{Criterio general de Abel}]\label{th:cga} Sean $(a_n)_{n=1}^\infty$\ y $(b_n)_{n=1}^\infty$\ dos sucesiones y supongamos que:\vspace{-3mm}
\begin{enumerate}
...
$\,\big\{\sumaf{j=1}{n}(b_j-b_{j+1})\big\}=\{b_1-b_{n+1}\}\,$, obtenemos que $(b_n)_{n=1}^\infty$\ es
...
\begin{proposicion} Si la sucesión $(b_n)_{n=1}^\infty$\ es monótona y acotada, entonces se verifica que es convergente la serie
$\sumasubn{1}|b_n-b_{n+1}|$.
\end{proposicion}
\dem En efecto, basta tener en cuenta que $$ \sumaf{j=1}{n}|b_j-b_{j+1}|=\begin{cases}
\sumaf{j=1}{n}(b_j-b_{j+1})=b_1-b_{n+1}, &\text{si $(b_n)_{n=1}^\infty$\ es decreciente;}\\
\sumaf{j=1}{n}(b_{j+1}-b_{j})=b_{n+1}-b_1, &\text{si $(b_n)_{n=1}^\infty$\ es creciente.}
\end{cases}
$$\fin
...
\begin{corolario}[\textbf{Criterio particular de Dirichlet}]\label{cpD} Sean $(a_n)_{n=1}^\infty$, $(b_n)_{n=1}^\infty$\ dos sucesiones, y pongamos $A_n=\dis\sum_{k=1}^n a_n$. Supongamos que:\vspace{-3mm}
\begin{enumerate}
\item[i)] Existe un número $M>0$ tal que para todo \nN\ es $\,\big|A_n\big|\leqslant M$.
\item[ii)] $(b_n)_{n=1}^\infty$\ es monótona y $$(b_n)_{n=1}^\infty$\to 0$.
\end{enumerate}
Se verifica entonces que la serie $\sumasubn{1}a_nb_n\,$ es convergente.
\end{corolario}

\begin{corolario}[\textbf{Criterio particular de Abel}] \label{cpA} Sean $(a_n)_{n=1}^\infty$, $(b_n)_{n=1}^\infty$\ dos sucesiones y supongamos que: \vspace{-3mm}
\begin{enumerate}
\item[i)] La serie $\sum a_n$ es convergente.
\item[ii)] $(b_n)_{n=1}^\infty$\ es monótona y acotada.
\end{enumerate}
Se verifica entonces que la serie $\sumasubn{1}a_nb_n\,$ es convergente.
\end{corolario}
...
\begin{proposicion}[\textbf{Criterio de Leibniz para series alternadas}] Supongamos que la sucesión $(a_n)_{n=1}^\infty$\ es
...
Sea $S=\sum_{n=1}^\infty (-1)^{n+1}a_n=\lim$(A_n)_{n=1}^\infty$$. Puesto que
$$
...
Teniendo en cuenta la proposición \ref{prop:seriescasiiguales}, el criterio de Leibniz prueba que las series de la forma $\sum(-1)^{n+1}a_n$ donde $$(a_n)_{n=1}^\infty$\to 0$ y la sucesión $(a_n)_{n=1}^\infty$\ \emph{es monótona a partir de un cierto término en adelante}, son convergentes (aunque la acotación del error antes obtenida ya no tiene por qué ser válida).
...
\end{array}
$$
\sol a)\ Pongamos $a_n=\dfrac{1}{n^{\alpha}+(-1)^{n}}$. Si $\alpha\le 0$ entonces $(a_n)_{n=1}^\infty$\ no converge a $0$ y la serie no converge. Supondremos en lo que sigue que $\alpha>0$. Tenemos que:
$$
...
c)\ Estudiaremos primero la convergencia absoluta. Sea $a_n=\left(\dfrac{1\cdot 3\cdot 5\cdots (2n-1)}{2\cdot 4\cdot 6\cdots 2n}\right)^{\!\alpha}$. Si $\alpha\le 0$ entonces $(a_n)_{n=1}^\infty$\ no converge a $0$ y la serie no es convergente. Supondremos en lo que sigue que $\alpha>0$. Tenemos que:
$$
...
\noindent\textbf{Series de diferencias o telescópicas.}\ Se llaman así las series $\sum a_n$ cuyo término general puede escribirse en la forma $a_n=b_{n+1}-b_n$. Puesto que, en tal caso, se verifica la igualdad $$\dis\sum_{k=1}^na_k=b_{n+1}-b_1,$$ la serie converge si, y sólo si, la sucesión $(b_n)_{n=1}^\infty$\ converge, en cuyo caso $\dis\sum_{n=1}^\infty a_n=\lim$(b_n)_{n=1}^\infty$-b_1$.
...
\noindent a)\ $(c_n)_{n=1}^\infty$\ es una sucesión de números enteros tales que $0\le c_n\le b-1$.\newline
b)\ \rule{0mm}{5mm}El conjunto $\set{\nN:c_n\neq b-1}$ es infinito.\newline
c)\  \rule{0mm}{5mm}$x=\dis\sum_{n=1}^\infty\dfrac{c_n}{b^n}$.

Además, si $(a_n)_{n=1}^\infty$\ es otra sucesión de números enteros tales que $0\le a_n\le b-1$ y $x=\dis\sum_{n=1}^\infty\dfrac{a_n}{b^n}$, entonces existe un $m\en\N$ tal que:
...
$$
\alpha_n=\sum_{k=1}^n\frac{c_k}{b^k}\ms{3mu}\longrightarrow\ms{3mu}x=\lim$(r_n)_{n=1}^\infty$=\sum_{n=1}^\infty\frac{c_n}{b^n}.
$$
...
\noindent Sea $(a_n)_{n=1}^\infty$\ una sucesión en las condiciones del enunciado. Definamos $m=\min\set{j\en\N:a_j\neq c_j}$. Por la definición de $m$ es claro que se verifica \emph{i)}. Para probar \emph{ii)} pongamos:
$$
%\begin{aligned}
...
Los criterios generales de Dirichlet y de Abel (teoremas \ref{th:cgd}\ y \ref{th:cga}) permanecen válidos sin cambio alguno para series de números complejos. Los criterios particulares de Dirichlet y de Abel (teorema \ref{cpD}\ y \ref{cpA}) pueden aplicarse igualmente a series de la forma $\dis\sum a_n b_n$, donde $(a_n)_{n=1}^\infty$\ es una sucesión de números complejos y $(b_n)_{n=1}^\infty$\ es una sucesión de números reales que satisfacen las hipótesis de dichos criterios.
...
ii)\ $\dis\modulo{\dfrac{\cos n+i\,\sen n}{n}}=\frac{1}{n}$. La serie no es absolutamente convergente. Para estudiar la convergencia no absoluta aplicaremos el criterio particular de Dirichlet (corolario \ref{cpD}). Pongamos $b_n=\frac{1}{n}$ y $a_n=\cos n+i\sen n=\e^{in}$. Tenemos que $(b_n)_{n=1}^\infty$\ es monótona y converge a $0$. Además:
$$
\begin{aligned}
...
Observa que el criterio particular de Dirichlet implica que las serie de números complejos de la forma $\serie{z^n b_n}$ donde $(b_n)_{n=1}^\infty$\ es una sucesión de números reales monótona y convergente a $0$ y $z$ es un número complejo de módulo $1$ y distinto de $1$, ($z\neq 1,\ \abs{z}=1$), son convergentes. Naturalmente si $\abs{z}<1$ tales series convergen absolutamente.\hecho
