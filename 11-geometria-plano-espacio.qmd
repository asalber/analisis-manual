...
## Producto escalar
Dados dos vectores no nulos $\mathbf{u}$ y $\mathbf{v}$ en $\mathbb{R}^n$, se cumple que
...
Si $\mathbf{u}=(u_1,u_2,u_3)$ y $\mathbf{v}=(v_1,v_2,v_3)$ son dos vectores no nulos ni paralelos en $\mathbb{R}^3$, el vector $\mathbf{u}\times \mathbf{v}$ es ortogonal a ellos.
:::
