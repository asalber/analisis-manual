...
donde $x_i(t)$, $i=1,\ldots,n$, son funciones reales de una variable real conocidas como _funciones coordenadas_ o _funciones componentes_.
:::
...
Dada una función vectorial  $\mathbf{f}(t)=(x_1(t),\ldots,x_n(t))$ en $\mathbb{R}^n$, se define el _límite_ de $\mathbf{f}$ cuando $t$ se aproxima a $a$ como el vector
...
Dada una función vectorial  $\mathbf{f}(t)=(x_1(t),\ldots,x_n(t))$ en $\mathbb{R}^n$, se dice que $\mathbf{f}$ es continua en $t=a$ si 
...
Una función vectorial  $\mathbf{f}(t)=(x_1(t),\ldots,x_n(t))$ en $\mathbb{R}^n$ es continua en $t=a$ si y solo si sus funciones componentes son continuas en $t=a$.
:::
...
## Derivada de una función vectorial
Se dice que una función vectorial $\mathbf{f}(t)=(x_1(t),\ldots,x_n(t))$ en $\mathbb{R}^n$ es _derivable_ o _diferenciable_ en un punto $t=a$ si existe el límite
...
Dada una función vectorial $\mathbf{f}(t)=(x_1(t),\ldots,x_n(t))$ en $\mathbb{R}^n$, si $x_i(t)$ es derivable en $t=a$ para cada $i=1,\ldots,n$, entonces $\mathbf{f}$ es derivable en $a$ y su derivada vale
...
:::{.callout-important}
En este contexto, la derivada de la función vectorial $f'(a)=(x_1'(a),\ldots,x_n'(a))$ es el _vector velocidad_ de la trayectoria descrita por $\mathbf{f}$ en el instante $t=a$.
:::
...
Los vectores paralelos a al vector velocidad $\mathbf{v}$ se denominan _vectores tangentes_ a la trayectoria de $\mathbf{f}$ en el instante $t=a$, y la recta que pasa por $P=f(a)$ dirigida por $\mathbf{v}$ es la recta tangente a $\mathbf{f}$ cuando $t=a$.
...
Hemos visto que para la función vectorial $\mathbf{f}(t) = (\cos(t),\operatorname{sen}(t))$, $t\in \mathbb{R}$, cuya cuya trayectoria es la circunferencia unitaria centrada en el origen de coordenadas, en el instante $t=\pi/4$ la posición del móvil era $f(\pi/4)=(\sqrt{2}/2,\sqrt{2}/2)$ y su velocidad $\mathbf{v}=(-\sqrt{2}/2,\sqrt{2}/2)$, de modo que la recta tangente a la trayectoria de $\mathbf{f}$ en ese instante es
...
Como acabamos de ver, la recta tangente a la trayectoria de la función vectorial $\mathbf{f}$ en $t=a$, está dirigida por el vector velocidad $\mathbf{v}=f'(a)=(x'(a),y'(a))$. Si en lugar de tomar ese vector se toma como vector director el vector $\mathbf{w}=(y'(a),-x'(a))$, que es ortogonal a $\mathbf{v}$, se obtiene otra	recta que se conoce como _recta normal a la trayectoria_.
...
y la recta tangente a la trayectoria de $\mathbf{f}$ en ese instante es
