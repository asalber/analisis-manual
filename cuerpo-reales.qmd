...
## Demostración
:::{.proof}
Vamos a demostrarlo por reducción al absurdo. Supongamos que no existe $n\in\mathbb{N}$ tal que $x<n$. Entonces, $x$ es una cota superior de $\mathbb{N}$. Por tanto, por el axioma del supremo existe un número $s=\sup(\mathbb{N})$. Por ser supremo, se cumple que existe un número natural $m\in\mathbb{N}$ tal que $s-1<m$, pero entonces se tiene que $s<m+1$, y como $m\in\mathbb{N}$ también $m+1\in\mathbb{N}$, lo que contradice que $s$ sea cota superior de $\mathbb{N}$. 
:::
:::
