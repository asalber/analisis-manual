...
De los axiomas de cuerpo algebraico de los números reales se deducen las siguientes consecuencias: 
...
a. Supongamos que existe $b\in\mathbb{R}$ tal que $b+a=a$ $\forall a\in\mathbb{R}$. Entonces $b+a+(-a)=a+(-a)$, y por el axioma 5 se tiene $b + 0 = 0$, de donde se deduce por el axioma 4 que $b=0$.

a. Supongamos que existe $b\in\mathbb{R}$ tal que $b\cdot a=a$ $\forall a\in\mathbb{R}$. Entonces $b\cdot a\cdot a^{-1}=a\cdot a^{-1}$, y por el axioma 10 se tiene $b \cdot 1= 1$, de donde se deduce por el axioma 9 que $b=1$.

a. Sea $a,b\in \mathbb{R}$ tales que $a+b=0$. Entonces $(-a)+a+b=(-a)+0$, y por los axiomas 4 y 5 se tiene $b=-a$.

a. Sea $a,b\in \mathbb{R}$ tales que $a\cdot b=1$. Entonces $(a^{-1})\cdot a\cdot b=a^{-1}\cdot 1$, y por los axiomas 9 y 10 se tiene $b=a^{-1}$.

a. Para cualquier $a\in\mathbb{R}$, usando los axiomas 4 y 11 se cumple $a\cdot 0= a\cdot (0+0)=(a\cdot 0)+(a\cdot 0)$. Si se suma ahora $-(a\cdot 0)$ a ambos lados de la igualdad y usando el axioma 5 se tiene

    $$
    0 =-(a\cdot 0)+(a\cdot 0) = -(a\cdot 0)+(a\cdot 0)+(a\cdot 0) = 0 + (a\cdot 0) = (a\cdot 0)
    $$

a. Para cualquier $a\in\mathbb{R}$, usando los axiomas 9, 10 y 11, se tiene $a+(-1)\cdot a = 1\cdot a + (-1)\cdot a = (1+(-1))\cdot a = 0\cdot a$. Usando ahora el resultado anterior $0\cdot a = 0$ de manera que $a+(-1)\cdot a = 0$, y por el axioma 5 se cumple que $(-1)\cdot a = -a$.

a. Para cualquier $a\in\mathbb{R}$, usando el axioma 5 se tiene $a+(-a)=0$ y por el axioma 3 $a+(-a)=(-a)+a=0$, de manera que usando de nuevo el axioma 5 se tiene $a=-(-a)$.

a. Para cualesquiera $a,b\in\mathbb{R}$, por la propiedad f se tiene $(-a)\cdot (-b)=(-1)\cdot a\cdot (-1)\cdot b = (-1)(-1)\cdot a\cdot  b= -(-1)\cdot a\cdot b$. Como $-(-1)=1$ según la propiedad anterior, aplicando el axioma 9 se tiene $(-a)\cdot(-b)=1\cdot a \cdot b=a\cdot b$. 
:::
:::
...
b. Supongamos que existe $b\in\mathbb{R}$ tal que $b\cdot a=a$ $\forall a\in\mathbb{R}$. Entonces $b\cdot a\cdot a^{-1}=a\cdot a^{-1}$, y por el axioma 10 se tiene $b \cdot 1= 1$, de donde se deduce por el axioma 9 que $b=1$.

c. Sea $a,b\in \mathbb{R}$ tales que $a+b=0$. Entonces $(-a)+a+b=(-a)+0$, y por los axiomas 4 y 5 se tiene $b=-a$.

d. Sea $a,b\in \mathbb{R}$ tales que $a\cdot b=1$. Entonces $(a^{-1})\cdot a\cdot b=a^{-1}\cdot 1$, y por los axiomas 9 y 10 se tiene $b=a^{-1}$.
...
<!-- TODO: Demostración propiedad ínfimo -->
...
De la propiedad arquimediana se deducen las siguientes consencuencias:
a. Si $a>0\in \mathbb{R}$, entonces $\exists n\in \mathbb{N}$ tal que $0<\frac{1}{n}<n$.
...
<!-- TODO: Demostración teorema raíz cuadrada -->
