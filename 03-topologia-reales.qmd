...
Veamos ahora que $a\leq b$. Para ello basta probar que $a$ es cota inferior de $B$. Para cualquier $n\in\mathbb{N}$ se cumple que si $k\geq n$ entonces $a_k\leq b_k\leq b_n$ pues $I_k\subseteq I_n$, y si $k<n$, entonces $a_k\leq a_n\leq b_n$, pues $I_n\subseteq I_k$. Luego $b_n$ es una cota superior de $A$, y por tanto, $a\leq b_n$ $\forall n\in\mathbb{N}$, de manera que $a$ es cota inferior de $B$. Así pues, como $a$ es cota superior de $A$ e inferior de $B$, se tiene que $a_n\leq a\leq b_n$ $\forall n\in\mathbb{N}$, de manera que $a\in \cap_{n=1}^\infty I_n$. 
